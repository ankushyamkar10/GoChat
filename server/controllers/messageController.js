...
const sendMsg = asyncHandler(async (req, res) => {
    ...
    if (isGrp) {
        ...
        const updatedGrp = await Group.findByIdAndUpdate({ _id: reciever }, { $push: { messages: newMsg } }, { new: true });
        if (updatedGrp) { res.status(200).json({ message: updatedGrp.messages.slice(-1)[0].message, isSenderMe: true }) } else { res.status(400).json({ msg: 'Failed to send msg' }) }
    }
    else {
...
const deleteAllMsg = asyncHandler(async (req, res) => {
    ...
    allGrps.forEach(async element => {
        await Group.findByIdAndUpdate({ _id: element._id }, {
            $set: { messages: [] }
        })
    });
})
